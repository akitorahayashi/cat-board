            ...
            AsyncThrowingStream { continuation in
                Task {
                    ...
                    for attempt in 0 ..< maxFetchAttempts {
                        ...
                        if !enableScreening {
                            // スクリーニングが無効な場合、ダウンロード成功した画像をすべて安全とみなし Yield する
                            let modelsToYield = successfulDownloadsForBatch.map(\.model)
                            if !modelsToYield.isEmpty {
                                print(
                                    "[LiveImageClient Stream] スクリーニング無効のため、APIページ \(currentPage) からダウンロード成功した \(modelsToYield.count)件の画像を Yield します。"
                                )
                                continuation.yield(modelsToYield)
                                yieldedSafeModelsCount += modelsToYield.count
                            }
                            currentPage += 1
                            continue // 次のフェッチ試行へ
                        }
