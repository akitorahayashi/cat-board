...
public struct LiveImageClient: ImageClientProtocol {
    ...
    public func fetchImages(desiredSafeImageCountPerFetch: Int, timesOfFetch: Int) async -> AsyncThrowingStream<[CatImageModel], Error> {
        AsyncThrowingStream { continuation in
            Task {
                ...
                print("[LiveImageClient Stream] 目標最大\(desiredSafeImageCountPerFetch)件の安全な画像取得を開始します。開始ページ: \(timesOfFetch)")
                ...
                for attempt in 0 ..< timesOfFetch {
                    ...
                    // 画像ダウンロード処理
                    let downloadResultsForBatch = await self.downloadImages(for: fetchedImageModels)
    ...
    private func performFetch(imageCountPerRequest: Int, pageNumber: Int) async throws -> [CatImageModel] {
        guard let url =
                URL(string: "https://api.thecatapi.com/v1/images/search?limit=\(imageCountPerRequest)&page=\(pageNumber)&order=Rand")
        else {
